...
const MAX_DAMAGE = 100000; // set higher if you truly want
...
function snapshot() {
  return Array.from(players.values()).map(p => ({
    id: p.id, x: p.pos.x, y: p.pos.y, hp: p.hp
  }));
}
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => {
    ...
    if (msg.t === "click") {
      ...
      let dmg = Math.round(spd);              // damage = speed
      ...
      for (const other of players.values()) {
        ...
        if (d <= HIT_RADIUS && d < best) { best = d; target = other; }
      }
