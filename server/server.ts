...
const lastPong = new Map<any, number>()
...
function killsNeededForLevel(level: number) {
	if (level <= 1) return 3
	let need = 3
	for (let l = 2; l <= level; l++) need = Math.floor(need * 1.5)
	return Math.max(1, need)
}

function applyLevelUp(p: Player) {
