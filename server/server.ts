...
const bumpPresence = (region: string, delta: number) => {
...
const broadcastPresence = (region: string) => {
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (buf) => {
    ...
    if (msg.t === "join") {
      ...
      if (meta.region && meta.region !== region) bumpPresence(meta.region, -1)
      meta.region = region || null
      meta.name = name || null
      clients.set(ws, meta)

      if (meta.region) bumpPresence(meta.region, 1)
      if (meta.region) ws.send(JSON.stringify({ t: "presence", region: meta.region, n: presence.get(meta.region) || 0 }))
      return
    }
  })
