...
const hpHud = document.getElementById("hpHud") as HTMLDivElement;
...
const levelHud = document.getElementById("levelHud") as HTMLDivElement;
...
function setLeaderboard(rows: LeaderRow[] | null) {
  ...
  for (let i = 0; i < Math.min(10, rows.length); i++) {
    ...
    if (nm === myName) left.classList.add("me");
...
function setFillGradientHorizontal(el: HTMLElement, baseHue: number) {
...
function setFillGradientVertical(el: HTMLElement, baseHue: number) {
  const normal = hsl(baseHue, 92, 55);
  const darker = hsl(baseHue, 92, 45);
  el.style.background = `linear-gradient(to bottom, ${darker} 0%, ${darker} 33.333%, ${normal} 33.333%, ${normal} 100%)`;
}
...
function updateLevelBar() {
  ...
  if (pct <= 0) levelBarInner.style.width = "14px";
  else levelBarInner.style.width = `${pct * 100}%`;
